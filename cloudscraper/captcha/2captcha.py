...
class captchaSolver(Captcha):
    ...
    def requestJob(self, jobID):
        ...
        response = polling2.poll(
            lambda: self.session.get(
                f"{self.host}/res.php",
                params={"key": self.api_key, "action": "get", "id": jobID, "json": "1"},
                timeout=30,
            ),
            check_success=_checkRequest,
            step=5,
        ...
        if response:
            ...
            raise CaptchaTimeout("2Captcha: Error failed to solve Captcha.")
    ...
    def requestSolve(self, captchaType, url, siteKey):
        ...
        data.update(
            {"method": "userrcaptcha", "googlekey": siteKey}
            if captchaType == "reCaptcha"
            else {"method": "hcaptcha", "sitekey": siteKey}
        )
        ...
        if response:
            ...
            raise CaptchaBadJobID("2Captcha: Error no job id was returned.")
    ...
    def getCaptchaAnswer(self, captchaType, url, siteKey, captchaParams):
        ...
        if not captchaParams.get("api_key"):
            raise CaptchaParameter("2Captcha: Missing api_key parameter.")
